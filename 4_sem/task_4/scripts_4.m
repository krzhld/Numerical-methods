...
hold on;
loglog(A(1, 1:n), A(4, 1:n), 'LineWidth', 3, 'Color', 'y');
hold on;
loglog(A(1, 1:n), A(2, 1:n), 'mo', 'MarkerSize', 7, 'LineWidth', 3);
hold on;

loglog(A(1, 1:n), A(4, 1:n), 'mo', 'MarkerSize', 7, 'LineWidth', 3);
title('Fact error(eps)');
xlabel('Accuracy');
ylabel('Error');
legend({'Fact error', 'Bisector', 'Correction'}, 'Location', 'southeast', 'Fontsize', 12);
